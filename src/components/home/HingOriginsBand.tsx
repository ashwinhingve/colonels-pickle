import { HingIllustration, MountainOriginIllustration } from "@/components/illustrations";
    ...
    <section className="relative overflow-hidden bg-cp-cream-muted py-14">
      ...
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex flex-col items-center gap-4 text-center">
          <HingIllustration aria-hidden className="h-12 w-12" />
          <p className="font-hindi text-xs font-bold uppercase tracking-[0.2em] text-cp-terracotta">
            The Soul of Every Jar
          </p>
          <h2 className="font-display text-2xl font-extrabold text-cp-text sm:text-3xl">
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-3">
          {ORIGINS.map((o) => (
            <div
              key={o.name}
              className="group rounded-2xl border border-cp-border bg-white p-6 text-center shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-cp-gold/50 hover:shadow-md"
            >
