...
export function AddressStep({ onNext, disabled }: AddressStepProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-cp-text mb-2">Delivery Address</h2>
        <p className="text-cp-text-muted">Select or add a delivery address</p>
      </div>

      {/* Loading State */}
      {isLoading && (
        <div className="text-center py-8">
          <div className="inline-block w-8 h-8 border-4 border-cp-olive border-t-transparent rounded-full animate-spin"></div>
          <p className="mt-2 text-cp-text-muted">Loading addresses...</p>
        </div>
      )}
            ...
            <div
              ...
              className={`border-2 rounded-lg p-4 cursor-pointer transition-all ${
                selectedAddressId === address._id
                  ? 'border-cp-olive bg-cp-olive-light'
                  : 'border-cp-border hover:border-cp-terracotta/50'
              }`}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="font-semibold text-cp-text">{address.fullName}</h3>
                    {address.isDefault && (
                      <span className="bg-cp-olive-light text-cp-olive text-xs px-2 py-0.5 rounded-full">
                        Default
                      </span>
                    )}
                  </div>
                ...
                <input
                  ...
                  className="mt-1 w-4 h-4 text-cp-terracotta focus:ring-cp-olive/25"
                />
              </div>
            </div>
          ))}
      ...
      {!isLoading && addresses.length === 0 && !showNewAddressForm && (
        <div className="text-center py-8 bg-cp-cream-dark rounded-lg border-2 border-dashed border-cp-border">
          <p className="text-cp-text-muted mb-4">No saved addresses found</p>
          <Button
            onClick={() => setShowNewAddressForm(true)}
            className="bg-gradient-to-r from-cp-terracotta to-cp-terracotta-deep"
          >
      ...
      {showNewAddressForm && (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 border-2 rounded-lg p-6 bg-white">
          <h3 className="font-semibold text-lg text-cp-text mb-4">Add New Address</h3>
              ...
              <div className="relative">
                ...
                {pincodeLookup.loading && (
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <div className="w-4 h-4 border-2 border-cp-olive border-t-transparent rounded-full animate-spin"></div>
                  </div>
                )}
              </div>
            </FormField>
            ...
            <div className="flex items-center">
              <label className="flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  {...register('isDefault')}
                  className="w-4 h-4 text-cp-terracotta border-cp-border rounded focus:ring-cp-olive/25"
                />
                <span className="ml-2 text-sm text-cp-text">Set as default address</span>
              </label>
            </div>
          </div>
      ...
      {!isLoading && addresses.length > 0 && !showNewAddressForm && (
        <Button
          variant="outline"
          onClick={() => setShowNewAddressForm(true)}
          className="w-full border-2 border-dashed border-cp-border hover:border-cp-terracotta"
        >
          + Add New Address
        </Button>
      )}
      ...
      {!showNewAddressForm && (
        <Button
          ...
          className="w-full py-6 text-lg bg-gradient-to-r from-cp-terracotta to-cp-terracotta-deep hover:from-cp-terracotta-deep hover:to-cp-terracotta"
        >
          Continue to Payment
        </Button>
      )}
